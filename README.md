...
## สลิปโอนเงิน — Gemini ช่วยเดา + เปลี่ยน Rich Menu อัตโนมัติหลังส่ง

**เดิม** ระบบจะจัดว่ารูป/ไฟล์ที่ user ส่งมาเป็น "สลิปโอนเงิน" หรือไม่ โดยเดาจาก **คำในแชทย้อนหลัง 24 ชม.** (เช่น "โอน", "ชำระ", "สลิป") ซึ่งพลาดได้ถ้า user ส่งสลิปมาเฉย ๆ โดยไม่มีข้อความประกอบ

**ตอนนี้** ถ้าตั้ง `GEMINI_API_KEY` ไว้แล้ว (ใช้ key เดียวกับฟีเจอร์ Gemini AI ตอบคำถามด้านล่าง) ระบบจะ**ส่งรูปให้ Gemini (vision) ดูจริง ๆ** ว่าหน้าตาเหมือนสลิปโอนเงิน/หลักฐานชำระเงินไหม (เช็คทุกรูปที่ user ส่งเข้ามา ไม่ต้องพึ่งคำในแชทอย่างเดียวอีกต่อไป) พร้อมประเมิน **confidence 0-100%** และลองอ่านยอด/ธนาคาร/เลขอ้างอิงเบื้องต้นให้ด้วย

- **ไม่ใช่การยืนยันที่เชื่อถือ 100%** — เป็นแค่ตัวช่วยกรอง+อ่านคร่าว ๆ เท่านั้น ไม่มี fraud-check เหมือน EasySlip ดังนั้นสลิปที่ Gemini เป็นคนอ่าน (ไม่ได้ผ่าน EasySlip) จะได้สถานะ **"ต้องตรวจเอง" เสมอ** ไม่มีทาง auto-verified ผ่านได้เอง — ต้องรอแอดมินกดยืนยันที่หน้า "สลิปโอนเงิน" (จะเห็นข้อความ `🤖 Gemini เดาว่าอาจเป็นสลิป (มั่นใจ NN%)`)
- ถ้าตั้ง `EASYSLIP_TOKEN` ไว้ด้วย ระบบจะยังใช้ EasySlip เป็นหลักสำหรับตรวจยอด/ธนาคารแบบเชื่อถือได้ (auto-verify ได้) — Gemini จะเสริมแค่เรื่อง **"นี่คือสลิปไหม"** (เผื่อ user ส่งรูปมาโดยไม่มี context ข้อความ) ไม่ได้แย่งงาน EasySlip
- ถ้า**ไม่ได้ตั้ง** `EASYSLIP_TOKEN` เลย ระบบจะใช้ผลอ่านจาก Gemini แทนทั้งหมด (ยอด/ธนาคาร/ref/วันที่) แต่ยังคงต้องให้แอดมิน confirm เองเสมอ
- ปิดฟีเจอร์นี้ได้ด้วยการไม่ตั้ง `GEMINI_API_KEY` — ระบบจะกลับไปใช้ heuristic แบบเดิม (เดาจากคำในแชท) เหมือนก่อนหน้านี้ทันที
- เรียก Gemini vision **ทุกรูป/ไฟล์ที่ user ส่งเข้ามา** (ไม่ใช่แค่ตอนคุยเรื่องเงิน) — ถ้ามีรูปเข้ามาเยอะควรพิจารณาโควตา/ค่าใช้จ่ายของ Gemini API ประกอบด้วย

พอระบบจัดว่ารูป/ไฟล์ที่ user ส่งมาเป็น "สลิปโอนเงิน" (ไม่ว่าจะจากคำในแชท, `slip_notify_all_images`, หรือ Gemini เดาว่าใช่) — ไม่ว่าผลตรวจอัตโนมัติจะเป็น verified/review/rejected ก็ตาม — ระบบจะ **link rich menu ของ user คนนั้นเป็น `SLIP_SUCCESS_RICHMENU_ID` ทันที** (default = เมนู "แล้วพบกัน", `richmenu-aa532223aad4fcd9d9878b219ad37714`) พร้อมบันทึกลง `richmenu_history` (source = `slip`)
